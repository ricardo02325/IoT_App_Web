...
@push('css')
    ...
    <style>
        ...
        #nombre-error, #particle-error, #ubicacion-error {
            color: red;
            font-size: 0.875em;
            display: none;
        }
                    ...
                    @foreach ($salones as $index => $salon)
                        ...
                        <div class="modal fade" id="editSalonModal{{ $salon->id }}" tabindex="-1" aria-labelledby="editSalonLabel{{ $salon->id }}" aria-hidden="true">
                            <div class="modal-dialog">
                                <form class="editSalonForm">
                                    <div class="modal-content">
                                        <div class="modal-header bg-primary text-white">
                                            <h5 class="modal-title" id="editSalonLabel{{ $salon->id }}">✏️ Editar {{ $salon->nombre }}</h5>
                                        ...
                                        <div class="modal-body">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control" value="{{ $salon->nombre }}" required>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label">Ubicación</label>
                                                <select class="form-control" required>
                                                    <option value="5D" {{ $salon->ubicacion == '5D' ? 'selected' : '' }}>Salón 5D - Edificio A</option>
                                                    <option value="LSE" {{ $salon->ubicacion == 'LSE' ? 'selected' : '' }}>Laboratorio de Sistemas Eléctricos de Potencia (LSE)</option>
                                                    <option value="LEM" {{ $salon->ubicacion == 'LEM' ? 'selected' : '' }}>Laboratorio de Electricidad y Magnetismo (LEM)</option>
                                                    <option value="LIOT" {{ $salon->ubicacion == 'LIOT' ? 'selected' : '' }}>Laboratorio de Internet de las Cosas (LIOT)</option>
                                                    <option value="D" {{ $salon->ubicacion == 'D' ? 'selected' : '' }}>Dirección (D)</option>
                                                    <option value="LM" {{ $salon->ubicacion == 'LM' ? 'selected' : '' }}>Laboratorio de Mecánica (LM)</option>
                                                </select>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label">ID del dispositivo Particle</label>
                                                <input type="text" class="form-control" value="{{ $dispositivo->device_id ?? '' }}">
                                            </div>
                                        </div>
